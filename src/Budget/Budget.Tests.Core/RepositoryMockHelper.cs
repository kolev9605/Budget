...
namespace Budget.Tests.Core
{
    ...
    {
        public static IRecordRepository SetupRecordRepository(RecordModel record)
        {
            var recordRepositoryMock = new Mock<IRecordRepository>() { DefaultValue = DefaultValue.Mock };

            recordRepositoryMock
                .Setup(x => x.CreateAsync<RecordModel>(It.IsAny<Record>(), It.IsAny<bool>()))
            ...
            recordRepositoryMock
                .Setup(x => x.BaseGetByIdAsync<RecordModel>(DefaultValueConstants.Common.Id))
                .Returns(Task.FromResult(record));

            recordRepositoryMock
                .Setup(x => x.UpdateAsync<RecordModel>(It.IsAny<Record>(), It.IsAny<bool>()))
                .Returns(Task.FromResult(record));

            recordRepositoryMock
                .Setup(x => x.DeleteAsync<RecordModel>(It.IsAny<Record>(), It.IsAny<bool>()))
                .Returns(Task.FromResult(record));

            return recordRepositoryMock.Object;
        }

        public static IAccountRepository SetupAccountRepository<TResult>(TResult account)
        {

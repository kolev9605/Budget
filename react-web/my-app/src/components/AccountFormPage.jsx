...
const AccountFormPage = ({ existingAccounts = [] }) => {
  ...
  const validateForm = () => {
    ...
    const duplicate = existingAccounts.some(
      (acc) => acc.name.toLowerCase() === formData.name.toLowerCase() && (!isEditing || acc.id !== id)
    );
    if (duplicate) newErrors.name = "Account name already exists";
